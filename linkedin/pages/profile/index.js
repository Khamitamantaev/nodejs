...
export default function Profile({ userData, dataPublic, dataPrivate})  {
          ...
          <div className="w-full md:w-3/12 md:mx-2">
            ...
            <div className="bg-white p-3 border-t-4 border-green-400">
              <div className="image overflow-hidden">
                <img className="h-33 w-33 mx-auto" src={`${userData.image}`} alt="" />
              </div>
              ...
              <div className="grid grid-cols-2">
                <div>
                  <div className="flex items-center space-x-2 font-semibold text-gray-900 leading-8 mb-3">
                    ...
                    <span className="tracking-wide">Pulic Trees</span>
                  </div>
                  {dataPublic ? 
                    <ul className="list-inside space-y-2">
                    {dataPublic.map(tree => (
                      <li key={tree._id}>
                      <div className="text-teal-600">{tree.name}</div>
                    </li>
                    ))}
                  </ul>: 
                  <ul className="list-inside space-y-2">
                    <li>
                      <div className="text-teal-600">Not found public trees</div>
                    </li>
                  </ul>
                  }
                </div>
                <div>
                  <div className="flex items-center space-x-2 font-semibold text-gray-900 leading-8 mb-3">
                    ...
                    <span className="tracking-wide">Private Trees</span>
                  </div>
                  {dataPrivate ? 
                    <ul className="list-inside space-y-2">
                    {dataPrivate.map(tree => (
                      <li key={tree._id}>
                      <div className="text-teal-600">{tree.name}</div>
                    </li>
                    ))}
                  ...
                  <ul className="list-inside space-y-2">
                    <li>
                      <div className="text-teal-600">Not found private trees</div>
                    </li>
                  </ul>
                  }
                </div>
...
export async function getServerSideProps(context) {
  ...
  const publictrees = await db
    .collection("trees")
    .find({ rootUser: user._id, isPrivate: false})
    .sort({ timestamp: -1 })
    .toArray()
    const privatetrees = await db
    .collection("trees")
    .find({ rootUser: user._id, isPrivate: true})
    .sort({ timestamp: -1 })
    .toArray()
  return {
    props: {
      session,
      dataPublic: publictrees.map((tree) => ({
        _id: tree._id.toString(),
        name: tree.name
      })),
      dataPrivate: privatetrees.map((tree) => ({
        _id: tree._id.toString(),
        name: tree.name
      })),
      userData: {
